...
$(document).ready(function() {
    ...
    if (searchFormValue) {
        $('#uploadForm_Search_by_Label').attr('checked', true);
        showSearchByLabel();
        $('#uploadForm_Label').val(searchFormValue);
    } else {
        $('#uploadForm_Search_by_Label').attr('checked', false);
        hideSearchByLabel();
    }
    
    $('#uploadForm_Search_by_Label').click(function() {
        if ($('#uploadForm_Search_by_Label').is(':checked')) {
            showSearchByLabel();
            $('#uploadForm_Label').val(searchFormValue);
        } else {
            hideSearchByLabel();
    ...
    $('#downloadDictionary').click(function() {
        ...
        if ((languageGroupId != "0") && (targetLanguageId != 0)) {
            ...
            $('#progress_image').show();
            setTimeout(function() {
                var flag = downloadDictionary('localization/downloadDictionary', targetLanguageId, languageGroupId);
                if (flag) {
                    $('#progress_image').hide();
                }
            }, 1000);
    ...
    $('#search').click(function() {
        $('#pageNo').val(0);
        if ($('#uploadForm_Search_by_Label').is(':checked')) {
            searchFormValue = $('#uploadForm_Label').val();
            $('#searchValue').val($('#uploadForm_Label').val());
        } else {
...
function validateUploadForm()
{
    ...
    checkMessageBar();
    if (lanGroup == '') {
        $(".messageBar").html("<span class='error'>Please Select Language Group</span>");
        checkMessageBar();
    }
    else if ((tarLan == '') && withTarget) {
        $(".messageBar").html("<span class='error'>Please Select a Lanuage to Translate</span>");
        checkMessageBar();
    }
    else if (file == '') {
        $(".messageBar").html("<span class='error'>Please Select a File</span>");
        checkMessageBar();
    }
    else {
        return true;
...
function validateAddSourceForm()
{
    ...
    if (label == '') {
        $(".messageBar").html("<span class='error'>Please Enter a Source</span>");
        checkMessageBar();
    }
    else if (lanGroup == '') {
        $(".messageBar").html("<span class='error'>Please Select a Group</span>");
        checkMessageBar();
    }
    else {
        return true;
...
function submitPage(pageNo) {
    ...
    if (searchFormValue != null) {
        searchFormValue = $('#searchValue').val();
    }
    $('form.pagination_enabled').submit();
}

function showSearchByLabel() {
    $('#uploadForm_Label').parent().parent().show();
    $('#add_label_language_group_id').parent().parent().hide();
}

function hideSearchByLabel() {
    $('#uploadForm_Label').parent().parent().hide();
    $('#add_label_language_group_id').parent().parent().show();
}
